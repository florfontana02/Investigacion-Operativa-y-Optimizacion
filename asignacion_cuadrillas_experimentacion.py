...
def resolver_lp(prob, configuracion):
    ...
    for key, value in configuracion.items():
        if value is not None:
            exec(f'prob.parameters.{key}.set({value})')
    ...
    valor_objetivo = prob.solution.get_objective_value()
    nodos_explorados = prob.solution.progress.get_num_nodes_processed()
    brecha = prob.solution.MIP.get_mip_relative_gap() 

    return tiempo_resolucion, valor_objetivo, nodos_explorados, brecha
...
# Valores a probar para cada parámetro
    parametros = {
        "mip.strategy.nodeselect": [0, 1, 2, 3],
        "mip.strategy.variableselect": [-1, 0, 1, 2, 3, 4],
        "preprocessing.presolve": [0, 1],
        "mip.strategy.heuristiceffort": [0, 0.5, 1, 2]
    }

    # Configuración base (valores por defecto)
    configuracion_base = {
        "mip.strategy.nodeselect": None,
        "mip.strategy.variableselect": None,
        "preprocessing.presolve": None,
        "mip.strategy.heuristiceffort": None
    }
    ...
    for parametro, valores in parametros.items():
        for valor in valores:
            ...
            tiempo, objetivo, nodos, brecha = resolver_lp(prob, configuracion)
